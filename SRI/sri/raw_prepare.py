...
def _companion_spatial(spatial_clouds, spatial_edges, members, components, labels, rng_id, scale):
    """Bridge/background correction for the selected adjacency spatial response.
    ...
    the adjacency response throughout observed and rotated controls.
    """
    ...
    base={mode:score(spatial_clouds,mode) for mode in ('adjacency',)}
...
def build_state(nodes,stars,draws=None,compute_covariance=True,components_override=None,velocity_center_method='component_median',use_uncertainty=True,spatial_mem_scale=None):
    ...
    result={
        'S_mem_adjacency':sm['adjacency']['value'],
        'S_mem_node_adjacency':sm['adjacency']['base'],
