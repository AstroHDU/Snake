...
class SRIConfig:
    ...
    def validate(self) -> None:
        if self.spatial_cen_width_model != 'smem_scatter':
            raise ValueError('Unknown spatial width model')
        if self.cross_aggregation != 'mean_residual':
            raise ValueError('Unknown Cross aggregation')
        if self.cross_cap_single_largest or not self.spatial_cen_tested_edge_floor:
            raise ValueError('Published method requires uncapped Cross and tested-edge width protection')
        if not isinstance(self.use_uncertainty,bool):
            raise ValueError('use_uncertainty must be boolean')
        if self.velocity_center_method != 'component_median':
        ...
        import math
        for name in ('h_cross_kms','outlier_prominence','affine_ridge'):
            if not math.isfinite(getattr(self,name)):raise ValueError(name+' must be finite')
        if isinstance(self.min_rv_sources,bool) or not isinstance(self.min_rv_sources,int):raise ValueError('min_rv_sources must be an integer')
