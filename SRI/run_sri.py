...
# ---------------------------------------------------------------------------
INPUT = None  # supply --input, or auto-detect one CSV in ./input/
...
def main(argv=None):
    ...
    if isinstance(bridge_scope_col,str) and bridge_scope_col.strip().lower() in ('none','null',''):
        bridge_scope_col=None
    if args.parent is not None and not bool(parent) and args.bridge_scope_col is None:
        bridge_scope_col=None
