...
def cen(points, clouds=None, tested_edge_floor=True, width_model='smem_scatter'):
    ...
    if clouds is not None:
        ...
        elif width_model=='projected_mad':
            widths=np.array([np.median(np.abs(np.asarray(c)@u-np.median(np.asarray(c)@u))) for c in clouds])
...
def cross_loss(x,v,ridge=0.1,cap_single_largest=True):
    """Legacy point-only Cross helper retained for compatibility/audit.

    The nominal scorer uses :func:`cross_loss_measurement` below.
    """
    x=np.asarray(x,float); v=np.asarray(v,float); n=len(x)
    if n<4:
        return {'D_cross_kms':np.nan,'status':'insufficient_centers','loss':np.empty(0)}
    W,ident=affine_weights(x,ridge)
    Q=np.eye(n)-W; a=np.sum(Q*Q,axis=1); r=Q@v
    loss=np.sum(r*r,axis=1)/a
    adjusted=loss.copy()
    if cap_single_largest and len(loss)>=2:
        adjusted=np.minimum(loss,np.partition(loss,-2)[-2])
    L=float(np.mean(adjusted))
    return {
        'D_cross_kms':float(np.sqrt(L)),'status':'computed','loss':loss,'adjusted_loss':adjusted,
        'residual':r,'W':W,'amplification':a,'prediction_identifiable':ident,
        'raw_rms_kms':float(np.sqrt(np.mean(np.sum(r*r,axis=1)))),
    }
...
def cross_loss_measurement(x,v,cov,h=5.0,ridge=0.1,cap_single_largest=True,aggregation='mean_residual'):
    ...
    The RMS option is a numerical diagnostic, not the released score.
    """
    ...
    if aggregation not in ('mean_residual','legacy_rms'):raise ValueError('Unknown Cross aggregation')
    if aggregation=='legacy_rms' and cap_single_largest and len(adjusted)>=2:
        adjusted=np.minimum(adjusted,np.partition(adjusted,-2)[-2])
    D=float(np.mean(np.sqrt(loss))) if aggregation=='mean_residual' else float(np.sqrt(adjusted.mean()))
    return {
