"""Entity-source velocity estimator and covariance in one common convention.

Contraction is upstream. Base-node labels and node summaries do not enter this
estimator. All scalar velocity centres/errors are recomputed from the pooled
original source observations. The retained RV information formula is unchanged.
The diagonal local-centre error model is an approximation, not a claim that a
heteroscedastic median has the sampling covariance of an inverse-weighted mean.
The same pooled-entity transported-member median defines the centre and its
Jacobian for covariance propagation. Median-rank changes are a first-order
approximation limitation, not a reason to revert to node weights.
No individual missing stellar RV is filled or added to the input table.
"""
