...
Explicit measured-source-median and joint-observation estimators are available
for diagnostic API comparisons, not selected by the public default pipeline.
"""
import hashlib
import numpy as np
import pandas as pd
...
def source_arrays(sources,complete_only_errors=False,use_uncertainty=True):
    ...
    # Same source coordinate transform as the original implementation.
...
def estimate_measured_entity(sources,method='component_median',draws=4096,use_uncertainty=True):
    ...
    g,H,y,e,masks,rvinfo,fallback=source_arrays(sources,complete_only_errors=(method=='measured_median'),use_uncertainty=use_uncertainty)
    complete=masks[0]&masks[1]&masks[2]
    ...
    if method=='measured_median':
        if not complete.any():return result
        velocities=np.einsum('nki,nk->ni',H[complete],y[complete])
        result.update(uvw=np.median(velocities,axis=0),valid=True,source_uvw=velocities)
    elif method=='component_median':
    ...
    elif method=='joint_observations':
        good=np.column_stack(masks)
        if use_uncertainty:good=good&np.isfinite(e)&(e>0)
        A=H[good];values=y[good];err=e[good] if use_uncertainty else np.ones(int(good.sum()))
        W=A/err[:,None];z=values/err
        if np.linalg.matrix_rank(W)<3:return result
        centre=np.linalg.lstsq(W,z,rcond=None)[0];normal=W.T@W
        residual=(values-A@centre)/err
        result['source_ids']=g.loc[good.any(axis=1),'source_id'].astype(str).tolist()
        result.update(uvw=centre,valid=True,
            chi2=float(residual@residual),ndof=max(0,len(values)-3))
    else:raise ValueError('Unknown independent 3D method')
    if use_uncertainty:
...
def prepare_measured_kinematics(entity_members,labels,method='component_median',use_uncertainty=True):
    ...
    return dict(uvw=np.array([x['uvw'] for x in estimates]),
        ...
        N_RV=np.array([x['N_3D'] if method=='measured_median' else x['N_RV'] for x in estimates]),
