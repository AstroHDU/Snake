...
def test_method2_nominal_needs_no_error_columns():
    _,g=inputs()
    a=estimate_measured_entity(g,method='joint_observations',use_uncertainty=False)
    for c in g:
        if 'error' in c:g[c]=np.nan
    b=estimate_measured_entity(g,method='joint_observations',use_uncertainty=False)
